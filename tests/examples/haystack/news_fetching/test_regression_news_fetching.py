...
TRACE_DIR = os.environ.get("RAGAAI_TRACE_DIR", "trace_haystack")
...
def run_news_fetching_agent():
    ...
    try:
        ...
        if trace_file_path and os.path.exists(trace_file_path):
            ...
            print(f"Warning: No trace file found after running news_fetching.py. Trying shell command...")
            
            # Make sure the trace directory exists
            trace_dir_path = os.path.join(Path(__file__).resolve().parent, TRACE_DIR)
            os.makedirs(trace_dir_path, exist_ok=True)
            
            shell_cmd = f"cd {Path(__file__).resolve().parent} && RAGAAI_TRACE_DIR={TRACE_DIR} {python_executable} {script_path} --info testing-with-openai"
...
def setup_traces():
    ...
    if not trace_file_path:
        print(f"No trace file found in {TRACE_DIR} directory, running news_fetching.py to generate it")
